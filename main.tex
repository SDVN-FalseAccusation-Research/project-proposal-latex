...
Figure \ref{fig:arch} depicts the integrated high-level architecture of the proposed SDVN framework.
As shown in Figure \ref{fig:arch}, the topmost layer in the system is the AI analysis layer, which serves as the intelligence and reporting component.
It takes in two general types of information as input: (i) the current network state (e.g., node status and trust level) and (ii) time series and event streams that are continually reported by the RSUs and the controllers (e.g., warning messages, repeated anomalies, and observed misbehavior).
The network is represented as a graph-based model consisting of connected entities (vehicles, RSUs, and controllers) to uncover suspicious relationships and traffic behavior.
This leads to a narrative explanation from a language-based reasoning component about what is happening, why it is suspicious, and what kind of response should be taken.
Once this is derived, the top AI analysis layer produces an attack detection decision and a report of the response, which is then sent to the enforcement path.

The blockchain layer is implemented on \textbf{Hyperledger Fabric} to keep a common, immutable record of security-sensitive updates across multiple controllers.
There are three smart contract modules (SC1--SC3) on this layer that coordinate different types of trust decisions:
\begin{itemize}
...
\textbf{C1 - Data Plane Compromise:} Attackers are capable of compromising $n_f$ vehicular nodes and RSUs, where $n$ is the total number of nodes in the network. Compromised nodes are able to perform routing and reputation voting in addition to malicious operations like sending false accusations or modifying packets.

\textbf{C2 - Control Plane Attack:} Attackers can compromise at most $n_c-1$ of the whole set of $n_c$ SDN controllers (e.g., 3 of 4). The blockchain ordering service tolerates up to $\lfloor(n_o-1)/3\rfloor$ Byzantine failures with BFT-SMaRt consensus, with further buy-in from the Controller Trust Evaluation Smart Contract \cite{yahiatene2018blockchain}.

\textbf{C3 - Wireless Security:} Wireless V2V and V2I communication can be eavesdropped on, altered, and replayed, or flooded (denial of service) over IEEE 802.11p DSRC channels.

\textbf{C4 - Limits on Computing Resources:} Attackers can be arbitrarily strong within classical limits. Post-quantum public-key schemes (FALCON-1024 and Kyber-1024) are used to mitigate brute-force risks, including future quantum threats. Keys are refreshed and replaced regularly to make classical cryptanalysis infeasible \cite{bensasson2018scalable}.

\subsection{Attack Scenarios}
\textbf{Scenario 1 - Single-Accuser Fabrication:} A trusted malicious node with high reputation creates false evidence against vehicles.

\textbf{Scenario 2 - Sybil-Amplified Flooding:} Several fabricated identities work together to overwhelm the reputation system with false reports to generate consensus artificially.

\textbf{Scenario 3 - Timing-Based Accusations:} Attackers take advantage of real network stress events (congestion, handovers) to cover malicious accusations as real failures.

\textbf{Scenario 4 - Evidence-Spoofing:} Malicious nodes generate fake corrupted packet logs, false routing traces, or manipulated sensor readings in order to accuse innocent nodes.

\section{System Assumptions}
The system assumes the following security and infrastructure assumptions:
\begin{itemize}
    \item \textbf{A1 - Cryptography:} Cryptographic primitives (FALCON-1024, Kyber-1024, RSA-2048, ECDH-256, AES-256, HMAC-SHA-256) are computationally unbreakable with proper implementation \cite{bensasson2018scalable}.
    \item \textbf{A2 - BFT:} BFT-SMaRt consensus can endure up to $\lfloor(n_o-1)/3\rfloor$ Byzantine orderers and guarantees safety and liveness \cite{castro1999practical}.
    \item \textbf{A3 - Bootstrap:} In the early stages of deployment, when a group of trusted controllers are present, credentials are distributed, and the blockchain is initialized.
    \item \textbf{A4 - Network:} Availability of underlying physical communication infrastructure (IEEE 802.11p, cellular backhaul) is adequate for safety-critical services.
    \item \textbf{A5 - Timestamps:} Each entity's clocks are loosely synchronized ($\pm$1 second) using GPS or NTP for detection of replay.
    \item \textbf{A6 - Hardware:} Sufficient computational resources (multi-core CPUs, HSMs) are present in vehicles or RSUs to perform cryptographic and ML operations.
    \item \textbf{A7 - Validation:} A trusted registration authority (vehicle manufacturer, station operator, or government agency) provides initial credentials that identify the physical vehicle.
    \item \textbf{A8 - Honest Majority:} Most participants are honest with high-trust endorsers who adhere strictly to protocol.
\end{itemize}


\section{Overview of False Accusation Attacks}
...
Figure~\ref{fig:overview_single_accuser_dv} illustrates the data-plane case by outlining the process.
First, in \textbf{(step 1) Position of the Attacker}, a malicious vehicle/RSU positions itself close to an honest node.
Then, in \textbf{(step 2) Fabrication of the Accusation}, it constructs a fake accusation packet. The attacker uploads this claim in
\textbf{(step 3) Reporting to Brain (The Upload)} via the RSU to the controller. In
\textbf{(step 4) The Decision (Processing)}, the controller handles the report and revises the trust decision.
...
In \textbf{(step 1) Target Identification}, the controller picks a non-hostile vehicle as its target. It then executes
\textbf{(step 2) Internal Reputation Fabrication} by establishing false evidence for the node. Without external checks,
the controller issues \textbf{(step 3) Blacklist Command}. In \textbf{(step 4) Network-Wide Propagation}, the blacklist is
distributed to RSUs and neighboring nodes. The sequence ends in \textbf{(step 5) Complete Isolation}, where the target is
...
Figure~\ref{fig:overview_sybil_dv} depicts a data-plane Sybil attack in a narrative format. In
\textbf{(step 1) Sybil Generation}, the attacker fabricates multiple false identities. The Sybils launch
\textbf{(step 2) The Flooding (The Attack)} by sending synchronized false accusation packets to the RSU. These accusations are passed along in
\textbf{(step 3) Reporting to Brain (The Upload)} to the controller, which sees them as originating from separate nodes. The controller carries out \textbf{(step 4) The Decision (Processing)},
viewing the number as consensus. The narrative concludes at \textbf{(step 5) The Consequence (The Block)}, where the
...
Figure~\ref{fig:overview_sybil_cv} offers the control-plane Sybil explanation where the controller fabricates consensus.
First, the controller invents fictitious identities (S1*, S2*, S3*). It then employs \textbf{(step 2) Insert Fabricated Reports} by injecting false accusations.
To make the consensus look real, the controller performs \textbf{(step 3) Data Retrieval for Trust Evaluation}, then applies
\textbf{(step 4) Trust Evaluation (Sybil-amplified consensus)} to justify a downgrade. The narrative concludes with
\textbf{(step 5) The Consequence (The Block)}, blacklisting the victim with controller orders.
...
This closed-loop workflow system is depicted in Figure~\ref{fig:simple_block}. The
\textit{Registration and Authentication Module} governs entity participation. The
\textit{Location Verification and Behavioral Monitoring} module actively monitors nodes and creates
\textit{logs and events}. It sends the logs/events to the \textit{Trust Update Core}, which analyzes
security-relevant evidence and updates the three trust functions: \textit{participation} (SC1),
\textit{reputation} (SC2), and \textit{controller trust} (SC3). These resultant validated values are
stored in the system \textit{trusted state/ledger} (the current trusted network state). At the same
time, the \textit{AI Analysis and Report} module uses the logs/events and the ledger trusted state
to identify anomalous behavior and to issue a potential action. The \textit{Mitigation and Enforcement}
module takes the suggested action via the control/monitoring path, and feeds the results back into
the trust update to ensure it is in line with the previously recorded actions.
...
Our core innovation relies on a permissioned blockchain platform to remove the single points of
failure associated with centralized SDN architectures \cite{sharma2017distblocknet}. The blockchain
is a distributed, tamper-proof ledger of authentication events, routing-related security decisions,
and reputation state changes, making it infeasible for adversaries to erase evidence of malicious
activity or alter past records. The ledger is shared among multiple SDN controllers that form a
consortium, where critical decisions are made through collaborative consensus among trusted peers
\cite{yahiatene2018blockchain}. Figure~\ref{fig:functional_diagram} summarizes how blockchain
capabilities are used for node admission (registration and authentication) and for enforcing trust
decisions.

Three types of smart contracts host the security policies. First, the Registration and
Authentication Smart Contract validates identity using Zero-Knowledge Proofs (ZKPs), enabling
vehicles/RSUs to prove legitimacy without revealing private identifiers \cite{bensasson2018scalable}.
As illustrated in Figure~\ref{fig:functional_diagram}, the workflow branches based on whether the
node is a \emph{first-time access}. For first-time access, a Registration Authority performs
out-of-band verification and checks physical identity attributes (e.g., VIN/serial number). After
successful verification, cryptographic credentials are issued and the node creates its key material
and ZKP-related artifacts before submitting an initial registration request (left branch of
Figure~\ref{fig:functional_diagram}). For returning nodes (right branch), the node generates a session
request, constructs a ZKP of identity, and sends an authentication request to the controller, which
then retrieves the node record from the blockchain (``Blockchain: Retrieve Node Record'') as shown
in Figure~\ref{fig:functional_diagram}. The ZKP validity decision (``ZKP Valid?'') ensures that only
registered entities proceed without exposing private identity information \cite{bensasson2018scalable}.

Second, the Reputation Management Smart Contract implements weighted endorser trust scoring so
false accusations have reduced influence on blacklist decisions \cite{alshaibani2023blockchain}. In
Figure~\ref{fig:functional_diagram}, this corresponds to the trust decision stage (``Trust Score
Sufficient?'') after ZKP verification. Reputation updates and punitive actions require endorsement
from multiple parties, with each endorser’s vote weighted by historical accuracy. New nodes and
previously malicious nodes carry minimal weight, preventing Sybil attackers from immediately
impacting reputation scores. Reputation changes require consensus from multiple high-trust
endorsers, typically agreement by at least two-thirds of weighted trust scores \cite{bessani2014state}.
If trust is insufficient, the system follows the ``Blacklisted?'' decision path: blacklisted nodes are
rejected, while borderline cases trigger enhanced verification (e.g., additional evidence checks)
before any access is granted, as captured in Figure~\ref{fig:functional_diagram}. This design prevents
a node from progressing from authentication into active participation unless both cryptographic
proof and trust policy conditions are satisfied.

Third, the Controller Trust Evaluation Smart Contract mitigates controller compromise through
continuous behavioral monitoring \cite{khan2017topology}. In Figure~\ref{fig:functional_diagram}, the
controller validates incoming authentication attempts (``Controller Validates'') and initiates
blockchain lookups and policy checks. To prevent a malicious/compromised controller from abusing
this role, random subsets of nodes validate whether the controller’s reported network topology
matches actual packet flows. Discrepancies trigger trust score reductions, and if controller trust
...
Finally, after successful verification and trust approval in Figure~\ref{fig:functional_diagram}, the
system issues session keys (``Issue Session Keys Encrypt with Kyber-1024'') and commits an immutable
record update (``Blockchain: Update Authentication Timestamp''). These events provide an auditable
trail of who joined, when they joined, and under what trust conditions. The same authenticated
admission event is forwarded to the learning layer (``GNN: Add Node to Network Graph''), ensuring
that the AI components operate on blockchain-consistent, verified participation states.
...
As illustrated in Figure~\ref{fig:simple_block}, all system components form a single uninterrupted closed-loop
cycle across monitoring, validation, analysis, and response. Vehicles periodically authenticate through the
\textit{Registration and Authentication Module} to ensure legitimacy without revealing sensitive identity
information. During each cycle, nodes exchange routing and status information through RSUs and controllers,
and field observations of anomalous behavior are captured by the \textit{Location Verification and Behavioral
Monitoring} module, generating the \textit{Logs and Events} stream shown in the diagram.

When an accusation arises, it is handed off through the controller path to the trust update engine. As depicted
by the \textit{Trust Update Core} and the Fabric blocks in Figure~\ref{fig:simple_block}, each claim is accompanied
by authenticating evidence (e.g., signatures, timestamps, and location-related proofs), which is validated before
any trust implications are applied. Validated updates then influence the Fabric trust functions: participation
management (SC1), reputation updates (SC2), and controller reliability assurance (SC3). Accepted updates are
committed to the \textit{Shared Ledger/World State} so all controllers and analysis elements observe a uniform
...
analysis component monitors the evolving network graph for suspicious structures and behaviors. When anomalies
are detected, the language-based reasoning component combines these findings with system logs to produce threat
assessments and recommended actions.

If the AI verdict and the decentralized validation outcome both indicate an attack, the response is applied
through the \textit{Mitigation and Feedback} block (Figure~\ref{fig:simple_block}). Controllers enforce the
selected actions in the field (e.g., restricting or isolating malicious nodes), while final decisions and
outcomes are recorded back into Fabric so that each subsequent cycle builds on verified history. Within the
same loop, SC3 continuously monitors controller reliability, enabling rapid and safe controller switching when
required.

This integrated design forces an adversary to overcome multiple independent protections---evidence-based
validation, decentralized agreement, trust scoring, and AI-driven anomaly detection---making reputation
manipulation and coordinated attacks significantly harder to succeed while preserving operational requirements
...
\chapter{Timetable and necessary tools}

\section{Timetable}
\include{timeline}

\section{Necessary tools}
The tools and technologies required for the implementation of the proposed Blockchain, ZKP, GNN, LLM-based
defense framework to mitigate false accusation attack in SDVN will be:
\begin{itemize}
    \item \textbf{Network Simulator 3 (NS-3)}, for SDVN modeling, V2V and V2I communication simulation, and
    framework assessment in different attack instances.
    \item \textbf{Hyperledger Fabric}, to implement the distributed ledger, register vehicles and controllers,
    and deploy smart contract for reputation and trust management system with BFT-SMaRT consensus mechanism.
    \item \textbf{PyTorch or TensorFlow}, for graph neural network deployment with temporal attention, to
    identify Sybil attacks and full false accusation instances.
    \item \textbf{Hugging Face Transformers}, for Large Language Model deployment to analyze controller logs
    and system alerts, understand syntax and semantics, and prepare incident reports.
    \item \textbf{libsodium or PyCryptodome}, for post-quantum algorithm implementation (FALCON-1024,
    Kyber-1024), and for digital signatures, key exchange and encryption.
    \item \textbf{zkSNARK Libraries (libsnark or circom)}, for privacy-preserving identity authentication.
    \item \textbf{Python with NetworkX and Pandas}, for graph creation, feature analysis and graph printing.
\end{itemize}
